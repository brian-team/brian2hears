...
class DelayFilterbank(FIRFilterbank):
    '''
    ...
    def __init__(self, sound, delays):
        ...
        FIRFilterbank.__init__(self, sound, coeficients)

class FIRAveragingFilter(FIRFilterbank):
    '''
    Parameters
    ----------
    sound
...
class IIRAveragingFilter(LinearFilterbank):
    '''
    ...
    def __init__(self, sound, Ntaps, method = 'explicit'):
        ...
        LinearFilterbank.__init__(self, sound, self.b, self.a)

# Pulled from brian.hears

class ApproximateGammatone(FilterbankCascade):
    '''
    ...
    ``bandwidth``
        List or array of corresponding filters bandwidth, one for each cf.
    ...
    def __init__(self, source, cf, bandwidth = None, order=4):
        ...
        a = np.dstack((np.array([b0, b1, b2]).T,)*order)
        b = np.dstack((np.array([a0, a1, np.zeros(len(cf))]).T,)*order)
        self.order = order
        
        FilterbankCascade.__init__(self,source, b, a)

           
class LogGammachirp(FilterbankCascade):
    '''
    ...
    def __init__(self, source, f,b=1.019,c=1,ncascades=4):
        ...
        #np.concatenate the gammatone filter coefficients so that everything is in cascade in each frequency channel
        self.filt_b=np.concatenate([self.gammatone_filt_b, self.asymmetric_filt_b],axis=2)
        self.filt_a=np.concatenate([self.gammatone_filt_a, self.asymmetric_filt_a],axis=2)
        
        FilterbankCascade.__init__(self, source, self.filt_b,self.filt_a)
...
#            self.impulse_response[ich,:]=self.impulse_response[ich,:]/sqrt(np.sum(self.impulse_response[ich,:]**2))
            self.impulse_response[ich,:]=self.impulse_response[ich,:]/np.sum(np.abs(self.impulse_response[ich,:]))        
...
class LinearGaborchirp(FIRFilterbank):
    ...
    def __init__(self,source, f, time_constant, c, phase=0): 
        ...
        t=np.arange(t_start,-t_start,1./self.samplerate)
...
class IIRFilterbank(LinearFilterbank):
    ...
    def __init__(self, source, nchannels, passband, stopband, gpass, gstop, btype, ftype):
        ...
        Wpassband = np.atleast_1d(Wpassband)
        Wstopband = np.atleast_1d(Wstopband)
        ...
        self.filt_a=self.filt_a.reshape(self.filt_a.shape[0],self.filt_a.shape[1])
        self.filt_b=self.filt_b.reshape(self.filt_b.shape[0],self.filt_b.shape[1])  
...
class Butterworth(LinearFilterbank):
    ...
    def __init__(self,source, nchannels, order, fc, btype='low'):
        Wn=fc.copy()
        Wn=np.atleast_1d(Wn) #Scalar inputs are converted to 1-dimensional arrays
        self.samplerate = 1./(source.dt)
        try:
...
class BiQuadratic(LinearFilterbank):
    ...
    The transfer function of the filters are like the np.ones of  all second-order linear filters
...
#        cf = np.atleast_1d(cf)
        self.samplerate= 1./(source.dt)
        w0 = 2*np.pi*cf/self.samplerate/second
        BW = 2./np.log(2)*arcsinh((1./2/Q))
        alpha = np.sin(w0)*np.sinh(np.log(2)/2 * BW * w0/np.sin(w0) )
        
        
        
        b_temp =   np.array([alpha,0,-alpha])/(1 + alpha)
        a_temp = np.array([1 + alpha,-2*np.cos(w0),1 - alpha])/(1 + alpha)
        
        print  b_temp,a_temp
        self.filt_b = np.tile(b_temp.reshape([3,1]),[1,1,1])               
        self.filt_a = np.tile(a_temp.reshape([3,1]),[1,1,1]) 
...
class AsymmetricCompensation(FilterbankCascade):
    '''
    ...
    def __init__(self, source, f, b=1.019, c=1,ncascades=4):
        ...
        for k in np.arange(ncascades):
            ...
            r=np.exp(-p1*(p0/p4)**(k)*2*np.pi*b*ERBw/self.samplerate) #k instead of k-1 because range 0 N-1
            Df=(p0*p4)**(k)*p2*c*b*ERBw

            phi=2*np.pi*np.maximum((f+Df), 0)/self.samplerate
            psy=2*np.pi*np.maximum((f-Df), 0)/self.samplerate
            ...
            vwr=np.exp(1j*2*np.pi*fn/self.samplerate)
